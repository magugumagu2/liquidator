...
contract LiquidatorTest is Test {
    ...
    IQuoterV2 constant quoter =
        IQuoterV2(0x3d4e44Eb1374240CE5F1B871ab261CD16335B76a);
    IL2Encoder constant encoder =
        IL2Encoder(0xceceF475167f7BFD8995c0cbB577644b623cD7Cf);
    // IAaveOracle constant oracle = IAaveOracle(0x2Cc0Fc26eD4563A5ce5e8bdcfe1A2878676Ae156);
    IPoolDataProvider constant dataProvider =
        IPoolDataProvider(0x2A0979257105834789bC6b9E1B00446DFbA8dFBa);
    MockOracle oracle;
    ...
    function testLiquidationCbethCollateral() public {
        ...
        (uint256 cbethRequired, , , ) = quoter.quoteExactOutput(
            swapPath,
            debtToCover
        );
    ...
    function setupLiquidatablePosition(
        SetupLiquidationPositionParams memory params
    )
        internal
        returns (uint256 debtToCover, uint256 expectedLiquidationReward)
    {
        ...
        (address collateralAToken, , ) = dataProvider.getReserveTokensAddresses(
            address(params.collateral)
        );
        ...
        (, , , uint256 bonus, , , , , , ) = dataProvider
            .getReserveConfigurationData(address(params.collateral));
        uint256 protocolFee = dataProvider.getLiquidationProtocolFee(
            address(params.collateral)
        );
        (
            ,
        ...
        uint256 baseCollateral = (debtAssetPrice *
            debtToCover *
            params.collateralUnit) /
            (params.collateralAssetPrice * params.debtUnit);
        ...
        uint256 bonusCollateral = collateralToLiquidate -
            percentDiv(collateralToLiquidate, bonus);

        uint256 liquidationProtocolFees = percentMul(
            bonusCollateral,
            protocolFee
        );
